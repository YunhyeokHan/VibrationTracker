...
from VibrationTracker.vib_conf import VIB_NODES, get_class_from_opcode, LISTBOX_MIMETYPE
from nodeeditor.node_node import Node

DEBUG = False
DEBUG_CONTEXT = False

class VibrationTrakerSubWindow(NodeEditorWidget):
    def __init__(self):
        super().__init__()
    ...
    def doEvalOutputs(self):
        ...
        for node in self.scene.nodes:
            if node.__class__.__name__ == 'VibNode_ImportImages':
                node.eval()
        for node in self.scene.nodes:
            if node.__class__.__name__ == 'VibNode_CalibrateCamera':
                node.eval()
        for node in self.scene.nodes:
            if node.__class__.__name__ == 'VibNode_EstimatePose':
                node.eval()
        for node in self.scene.nodes:
            if node.__class__.__name__ == 'VibNode_InitializeTarget':
                node.eval()

        for node in self.scene.nodes:
            if node.__class__.__name__ == 'VibNode_PreprocessDIC':
                node.eval()
        
        for node in self.scene.nodes:
            if node.__class__.__name__ == 'VibNode_ProcessDIC':
                node.eval()
        
        for node in self.scene.nodes:
            if node.__class__.__name__ == 'VibNode_PostprocessDIC':
                node.eval()
                
        for node in self.scene.nodes:
            if node.__class__.__name__ == 'VibNode_TrackTarget':
                node.eval()

        for node in self.scene.nodes:
            if node.__class__.__name__ == 'VibNode_CalculateDisplacement':
                node.eval()
